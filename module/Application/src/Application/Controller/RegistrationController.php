...
class RegistrationController extends AbstractActionController implements EntityManagerAwareInterface
{
		...
		if ($this->getRequest()->isPost())
		{
			...
			if ($form->isValid())
			{
				...
				try
				{
					...
					$this->sendEmail(
							$user->getEmail(), $this->getTranslatorHelper()->translate('Please, confirm your registration!'), sprintf($this->getTranslatorHelper()->translate('Please, click the link to confirm your registration => %s'), $fullLink)
					);
				...
				} catch (\Exception $e)
				{
					return $this->getServiceLocator()->get('csnuser_error_view')->createErrorView(
									$this->getTranslatorHelper()->translate('Something went wrong when trying to send activation email! Please, try again later.'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu()
					);
				}
			}
		}
	...
	public function resetPasswordAction()
	{
		...
		if ($this->getRequest()->isPost())
		{
			...
			if ($form->isValid())
			{
				...
				$entityManager = $this->getEntityManager();
				$user = $entityManager->createQuery("SELECT u FROM Application\Entity\User u WHERE u.email = '$usernameOrEmail' OR u.username = '$usernameOrEmail'")
				...
				if (isset($user))
				{
					try
					{
						...
						$this->sendEmail(
								$user->getEmail(), $this->getTranslatorHelper()->translate('Please, confirm your request to change password!'), sprintf($this->getTranslatorHelper()->translate('Hi, %s. Please, follow this link %s to confirm your request to change password.'), $user->getUsername(), $fullLink)
						);
					...
					{
						return $this->getServiceLocator()->get('csnuser_error_view')->createErrorView(
										'Something went wrong when trying to send activation email! Please, try again later.<br>'
										. '<a href="' . $fullLink . '">' . $fullLink . '</a>', $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu()
						);
					}
				}
				else
	...
	public function confirmEmailAction()
	{
		...
		{
			return $this->getServiceLocator()->get('csnuser_error_view')->createErrorView(
							$this->getTranslatorHelper()->translate('Something went wrong during the activation of your account! Please, try again later.'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu()
			);
		}
	}
	...
	public function confirmEmailChangePasswordAction()
	{
		...
		{
			$entityManager = $this->getEntityManager();
			if ($token !== '' && true == $user = $entityManager->getRepository('Application\Entity\User')->findOneBy(array('registrationToken' => $token)))
			{
				$user->setRegistrationToken(md5(uniqid(mt_rand(), true)));
				$password = $this->generatePassword();
				$user->setPassword(UserCredentialsService::encryptPassword('123123'));
				$entityManager->persist($user);
				$entityManager->flush();
				$email = $user->getEmail();
				$fullLink = $this->getBaseUrl() . $this->url()->fromRoute('index', array('action' => 'login'));
				$this->sendEmail(
						$user->getEmail(), 'Your password has been changed!', sprintf($this->getTranslatorHelper()->translate('Hello again %s. Your new password is: %s. Please, follow this link %s to log in with your new password.'), $user->getUsername(), $password, $fullLink)
				);
